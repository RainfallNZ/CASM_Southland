...
REC_Catchments         <- file.path(GISDataDirectory,"ES_REC2_Watersheds","ES_REC2_Watersheds.shp") #An ESRI polygon shape file with Environment Southland Plan Classses
...
CASMBasinLookupTableFile<- file.path(DataDirectory, "CASMBasinToSoouthlandFMULookupTable.csv")
...
```
"Correct"the Waiau flows below Manapouri to account for most of the water going out to Doubtful Sound throught he power scheme.
...
NamedCompleteSpatialNetwork <- RECReachNamer(RECV2Network = CompleteSpatialNetwork)
...
NamedCompleteSpatialNetwork$name[match(ExtraRECReachLabels$nzsegment,NamedCompleteSpatialNetwork$nzsegment)] <- ExtraRECReachLabels$name
...
writeOGR(CompleteSpatialNetwork, file.path(GISDataDirectory), "CASM-StreamNetwork-named", driver="ESRI Shapefile",overwrite_layer=TRUE)
...
PointSourceTable$"N Annual Load (kg/yr)" <- round(PointSourcesSpatial$TN__t_y_[match(PointSourceTable$nzsegment,PointSourcesSpatial$nzsegment)]*1000,0)
PointSourceTable$"P Annual Load (kg/yr)" <- round(PointSourcesSpatial$TP__t_y_[match(PointSourceTable$nzsegment,PointSourcesSpatial$nzsegment)]*1000,0)
...
AssessmentSiteNodes$NodeName[is.na(AssessmentSiteNodes$NodeName)] <- CompleteSpatialNetwork$Label[match(AssessmentSiteNodes$nzsegment[is.na(AssessmentSiteNodes$NodeName)],CompleteSpatialNetwork$nzsegment)]
...
#Add the mean annual flow to the table
AssessmentSiteTable$"Mean flow (m3/s)" <- round(REC2MeanFlow$QMean[match(AssessmentSiteTable$nzsegment, REC2MeanFlow$nzsegment)],1)

#Update with the modified Waiau flows
WaiauIndices <- which(AssessmentSiteTable$nzsegment %in% WaiauFlowCorrection$nzsegment)
AssessmentSiteTable$`Mean flow (m3/s)`[WaiauIndices] <- WaiauFlowCorrection$QMean[match(AssessmentSiteTable$nzsegment[WaiauIndices],WaiauFlowCorrection$nzsegment)] 
...
#Any sites missing names, use their river reach names
DiffuseSiteNodes$NodeName[is.na(DiffuseSiteNodes$NodeName)] <- CompleteSpatialNetwork$Label[match(DiffuseSiteNodes$nzsegment[is.na(DiffuseSiteNodes$NodeName)],CompleteSpatialNetwork$nzsegment)]
...
#Add the CASM Basin details
DiffuseSiteTable$CASMBasin <- CompleteSpatialNetwork$CASMBasin[match(DiffuseSiteTable$nzsegment,CompleteSpatialNetwork$nzsegment)]
...
if(file.exists(DiffuseLoadSourceAreasFile)) {
  ...
  DiffuseSourceAreasOK <- identical(as.integer(sort(unique(DiffuseSiteTable$nzsegment))),as.integer(sort(unique(DiffuseLoadSourceAreas$nzsegment))))
}
...
if (!DiffuseSourceAreasOK) {
  ...
  DiffuseLoadSourceAreas <- CASMNodeSourceAreaGenerator(RECWatersheds = RECWatersheds,RECNetwork = RECReachNetwork@data, CASMNodes = DiffuseSiteTable$nzsegment)
...
#Calculate a new raster by dividing the DiffuseSourceArea by 100000000, then adding the landuse class.
...
#Now need to get the leach rates fr each of the nzsegment-landuse areas.
#Load the Leachrate Data
LeachRateRaster <- stack(LeachRateRasterFile)
#Get the total nitrogen leach rates for each unique combination of source area and land use
DiffuseLeachRateData <- DiffuseLoadTableCreator(ZoneLanduseLUCRaster = DiffuseSourceLanduse,LeachRates = LeachRateRaster[[2]])
...
DiffuseLeachRateData$TP_LeachRate <- DiffuseLoadTableCreator(ZoneLanduseLUCRaster = DiffuseSourceLanduse,LeachRates = LeachRateRaster[[4]])$LeachRate
...
DiffuseLeachRateData$'Node Name' <- paste0(DiffuseLeachRateData$`Site Name or No`,"-",DiffuseLeachRateData$nzsegment,"-",DiffuseLeachRateData$landuse)
...
DiffuseLeachRateData$AdjustedTriblocn <- DiffuseLeachRateData$'Downstream Location (km)'
UniqueSourceAreas <- unique(DiffuseLeachRateData$CombinedClassNameLevel)
for(SourceAreaIndex in seq_along(UniqueSourceAreas)) {
  ...
  RowsOfInterest <- which(DiffuseLeachRateData$CombinedClassNameLevel == SourceArea)
  DiffuseLeachRateData$AdjustedTriblocn[RowsOfInterest] <- DiffuseLeachRateData$'Downstream Location (km)'[RowsOfInterest] + seq(0,by = 0.01, length.out = length(RowsOfInterest))
}

#Copy it all to a new dataframe
DiffuseInputsSiteExtendedTable <- DiffuseLeachRateData

#rename the columns to match the CASM conventions
names(DiffuseInputsSiteExtendedTable) <- c("nzsegment","SoureAreaCode","TN Export Coeff (kg/ha/yr)","NoOfRasterCells","Land Area (ha)","TP Export Coeff (kg/ha/yr)","LanduseCode","Landuse","Site Name or No.","Receiving Stream","Original location (km)","Mean flow (m3/s)","CASMBasin","Node Name","Discharge Location (km)")
...
RECForMaps <- RECForMaps[,c("LineWidthPixels","WaterPlan","Label")]
...
writeData(Out, sheet = "Diffuse Inputs", x = DiffuseInputsSiteExtendedTable[,c("nzsegment","Node Name","Receiving Stream","Discharge Location (km)","Land Area (ha)","TN Export Coeff (kg/ha/yr)","TP Export Coeff (kg/ha/yr)")])
