...
IndicesOfnzsegmentsToUpdate <- which(AllPoints %in% as.character(CASMnzsegmentOverrideData$nzsegment))
AllPoints[IndicesOfnzsegmentsToUpdate] <- as.character(CASMnzsegmentOverrideData$CASMnzsegment)
...
Turn this into a function that accepts a spatial network and returns a labelled version
```{r}
...
#writeOGR(CompleteSpatialNetwork, file.path(GISDataDirectory), "CASM-StreamNetwork-namedV2", driver="ESRI Shapefile",overwrite_layer=TRUE)
...
#Get the mean flow for all the assessment points
#Add the mean annual flow to the table
MeanFlows <- REC2MeanFlow[match(AssessmentSiteTable$nzsegment, REC2MeanFlow$nzsegment),c("nzsegment","QMean")]
...
AssessmentSiteTable$"Mean flow (m3/s)" <- bob
...
#Calculate a new raster by multiplying the DiffuseSourceArea by 100, then adding the landuse class.
