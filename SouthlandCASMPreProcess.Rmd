...
LakePolygonFile <- "NZLRIClip_Lakes" #An ESRI polygon shape file
...
MeasurementSites <- readOGR(dsn =file.path(GISDataDirectory,WaterQualityStationFile),
...
LakeSites <- readOGR(dsn =file.path(GISDataDirectory,LakeSiteFile),
                           layer = LakeSiteFile, stringsAsFactors = FALSE)
LakeSites <- spTransform(LakeSites,CRS("+init=epsg:2193") )

#Load the lake polygon layer
Lakes <- readOGR(dsn =file.path(GISDataDirectory,LakePolygonFile),
                           layer = LakePolygonFile, stringsAsFactors = FALSE)
...
SubCatchmentConfluencePoints <- SubCatchmentConfluencePoints[,c("nzsegment","downcoordX","downcoordY")]
SubCatchmentConfluencePoints$Reason <- "Base of catchment"

#Turn into a spatial dataframe
SubCatchmentConfluencePointsSpatial <- SpatialPointsDataFrame(coords = SubCatchmentConfluencePoints[,c("downcoordX","downcoordY")],
...
#Remove points not withon the overall domain
...
AllPoints <- c(MeasurementSites$nzsegment, LakeSites$nzsegment, EstuarySites$nzsegment, SubCatchmentConfluencePointsSpatial$nzsegment, OutletReaches = RiverMouthSites$nzsegment)
...
Get some descriptive statistics of how well the points represent the important land uses
Find out how many points are in each of the Water Plan Classes.
and plot a histogram for each catchment showing the % pasture coverage
```{r, fig.width=12, fig.height = 12}
AllPointWaterPlan <- RECReachNetwork@data$WaterPlan[RECReachNetwork$nzsegment %in% AllPoints]
#kable(table(AllPointWaterPlan))

#Find which major catchment each point is in
AllPointReaches <- RECReachNetwork[RECReachNetwork$nzsegment %in% AllPoints,]

AllPointCatchments <- AllPointReaches %over% MajorCatchments
names(AllPointCatchments) <- "Catchments"
...
AllPointCatchments$Catchments[which(is.na(AllPointCatchments))] <- "Oreti & Invercargill Catchments"

AllPointspctExoticPasture <- SouthlandREC2Utility$pctExoticPasture[SouthlandREC2Utility$nzsegment %in% AllPoints]
hist(AllPointspctExoticPasture,xlab = "% of upstream catchment in high producing exotic grassland",ylab ="Number of points",main="")
GraphData <- data.frame(pctExoticPasture = AllPointspctExoticPasture, Catchment = AllPointCatchments)
#ggplot(GraphData, aes(x=pctExoticPasture,color = Catchments, fill = Catchments))+ geom_histogram(bins=10,position="dodge")

ggplot(GraphData, aes(pctExoticPasture)) + geom_histogram(bins=10,colour="grey",fill="white")

ggplot(GraphData, aes(x=pctExoticPasture))+ geom_histogram(bins=10,colour="grey",fill="white") + facet_wrap(. ~Catchments,ncol=2)
```
...
SpatialData <- list(MeasurementSites=MeasurementSites,LakeSites=LakeSites,EstuarySites=EstuarySitesSpatial,SubCatchmentSites=SubCatchmentConfluencePointsSpatial,RiverMouthSites=RiverMouthSites, RiverNetwork=RECForMaps,MajorCatchments=MajorCatchments,GroundwaterZones=GroundwaterZones)
