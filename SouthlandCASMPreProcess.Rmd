---
title: "R Notebook to preprocess water quality data for use with SCAMP, for Environment Southland"
output: html_notebook
---

In the begining there was CASM Catchment something something Model.
Then SCAMP was created. These scripts were originally developed for CASM, then re-purposed for use with SCAMP.
Hence the mishmash of CASM/SCAMP throughout.

load some libraries
```{r}
options("rgdal_show_exportToProj4_warnings"="none")   #suppresses the annoying warnings about datums and stuff.
...
if (!require(gdalUtils)) install.packages("gdalUtils"); library(gdalUtils)                #dependency of CASMPrePostProcessor
if (!require(cleangeo)) install.packages("cleangeo"); library(cleangeo)                #dependency of CASMPrePostProcessor
...
source("D:\\Projects\\LWP\\SouthlandRegionalForumModelling\\R\\CASM_Southland\\SouthlandSCAMPFunctions.R")
...
# Scenario <- "Wetland3"
# Scenario <- "Wastewater"
# Scenario <- "Retirement1"
# Scenario <- "Retirement2"
# Scenario <- "Restoration"
# Scenario <- "PotentialForestry"
# Scenario <- "PlantainAndOLW"
# 
# Scenario <- c("Wetland1","Wastewater","Restoration")
# Scenario <- c("OurLandWater2035Potential","Wetland1","Wastewater","Restoration")
# Scenario <- c("Wetland2","Wastewater","Restoration")
# Scenario <- c("OurLandWater2035Potential","Wetland2","Wastewater","Restoration")
# Scenario <- c("Wetland3","Wastewater","Restoration")
# Scenario <- c("OurLandWater2035Potential","Wetland3","Wastewater","Restoration")
# Scenario <- c("OurLandWater2035Potential","Wetland1","Wastewater","Restoration","Retirement1")
# Scenario <- c("OurLandWater2035Potential","Wetland1","Wastewater","Restoration","Retirement2")
# Scenario <- c("Wetland1","PlantainAndOLW","Wastewater","PotentialForestry","ESLand")
```
...
ScenarioLandUseGISDirectory = "D:\\Projects\\LWP\\SouthlandSCAMP\\Data\\GIS\\ScenarioLandUse"

#Set the output file name. If a scenario is being run, then the scenario name will be tacked on to the end of the file name
OutputBaseFileName <- "SCAMP-InputsV4_1Test"
if (Scenario == "Default"){
OutputFileName <- file.path(DataDirectory,paste0(OutputBaseFileName,".xlsx"))
} else {
  OutputFileName <- file.path(DataDirectory,paste0(OutputBaseFileName,"_",paste(Scenario,collapse=""),".xlsx"))
}

#Set some R data file names from the LWP data store
...
LanduseShapeFile        <- file.path(GISDataDirectory,"LanduseLayers/LandUse2017/LandUse2017Clean.shp")
...
PrecipIrrigShapeFile    <- file.path(GISDataDirectory,"LanduseLayers/PrecipitationClass_withIrri/Complete_Southland_Precip_Layer.shp")
...
#LeachRateRasterFile     <- file.path(GISDataDirectory,"LeachRateRasterV2 250 x 250.tif")
LeachRateRasterFile     <- file.path(GISDataDirectory,"LeachRateRasterV4_1 250 x 250.tif") #upgraded after provision of Version 4 look up table
#PredictorRasterFile     <- file.path(GISDataDirectory,"PredictorRaster 250 x 250.tif")
PredictorRasterFile     <- file.path(GISDataDirectory,"PredictorRasterV4_1 250 x 250.tif") #upgraded after provision of Version 4 look up table
...
RECUpdateDataFile       <- file.path(DataDirectory,"RECDataUpdates.csv")
...
WaterQualityDataFile    <-file.path(DataDirectory,"WaterQualityStationLoadConcDataV2.csv")
...
if (any(Scenario %in% c("Wastewater"))) PointSourceSitesFile    <- file.path(DataDirectory,"SldPointSourceLoadsNZSegmentAssigned_WastewaterScenarioV1.csv")
...
#CASMnzsegOverrideFile   <- file.path(DataDirectory,"CASMnzsegmentOverideForLocationV2.csv") Implemented fo SCAMP Input Version3B but ignored by Tim Cox, Caused mismatch between my data, SCAMP data and Ton's REC data.To try to align the data again this file was rolled back to V1 and SCAMP input changed to version 3C. 
CASMnzsegOverrideFile   <- file.path(DataDirectory,"CASMnzsegmentOverideForLocationV3.csv")
...
#LeachRateLUTFile        <- file.path(DataDirectory,"Losses Lookup Table V3_1.xlsx")   #Updated early June 2021
LeachRateLUTFile        <- file.path(DataDirectory,"Losses Lookup Table V4_1.xlsx")     #Updated 23rd August 2023 from "Loss Lookup Table - Updated March 2022.xlsx" provided by Ewen, and expandede "All" classes on 1st September
PhysiographicAttenuationLookupTableFile <- file.path(DataDirectory,"Relative Attenuation Estimates.xlsx")
#Water quality load data updated for location shifts
WaterQualityDataUpdateFile <- file.path(DataDirectory,"July2020_loads_at_locationswithnotablydifferentnzsegmentV2.xlsx" )

if (any(Scenario %in% c("OurLandWater2015Potential","OurLandWater2035Potential","PlantainAndOLW"))){
  OLWMitigationDataFile = "D:\\Projects\\LWP\\SouthlandRegionalForumModelling\\Data\\OurLandAndWaterMitigationLoadsV2.csv"
  OLWDairyLandTypesSpatialFile = "D:\\Projects\\LWP\\SouthlandRegionalForumModelling\\Data\\GIS\\OLW_Southland_TypologiesDairy\\OLW_Southland_TypologiesDairy.shp"
  OLWSheepAndBeefLandTypeSpatialDataFile = "D:\\Projects\\LWP\\SouthlandRegionalForumModelling\\Data\\GIS\\OLW_Southland_TypologiesSnB\\OLW_Southland_TypologiesSnB.shp"
}
if (any(Scenario %in% c("PlantainAndOLW"))){
  OLWMitigationDataFile = "D:\\Projects\\LWP\\SouthlandRegionalForumModelling\\Data\\OurLandAndWaterMitigationLoadsV2WithPlantainEffect.csv"
}
ESLandPolygonFile <- "D:\\Projects\\LWP\\SouthlandSCAMP\\Data\\GIS\\ESLand\\ESLand.shp"
```
...
#REC data has been sourced from LWP data stores.
#Unfortunately some of the associated attributes have not been updated after the network has been.
#This is known to affect the "headw_dist" attribute
if (!exists("RECReachNetwork")){
  ...
  #Explicitly set the "headw_dist","nzsegment","TO_NODE","FROM_NODE" attributes to numeric
  ...
  #Load a manually prepared update file which has manual estimates for corrected attributes for some segments
  RECUpdateData <- read.table(RECUpdateDataFile,stringsAsFactors = FALSE, sep=",",header=TRUE)
  
  #Update the REC network data with the updates
  RECReachNetwork$headw_dist[match(RECUpdateData$nzsegment,RECReachNetwork$nzsegment)] <- RECUpdateData$headw_dist
...
#Note that SouthlandREC2Utility$catAreaKM2 is out of date and should be updated with the equivalent attribute from the REC network data
...
#Load the mean flow data for the REC V2. This is provided in an rdata file which contains the data frame REC2MeanFlow, with three columns, "nzsegment","QMean", and "us_catarea". Note that "us_catarea" is out of date. The CUMAREA attribute from the REC network data should be used.
load(REC_MeanFlowFile)

#Update the QMean using the manually prepared RECUpdateData
REC2MeanFlow$QMean[match(RECUpdateData$nzsegment,REC2MeanFlow$nzsegment)] <- RECUpdateData$Qmean
...
#Load Measurement site data update data
WaterQualityDataUpdate <- read.xlsx(WaterQualityDataUpdateFile)
#Update the MeasurementSite data
MeasurementSites$`TN Load(kg/yr)`[match(WaterQualityDataUpdate$site.name,MeasurementSites$sID)] <- WaterQualityDataUpdate$TN_kg
MeasurementSites$`TP Load(kg/yr)`[match(WaterQualityDataUpdate$site.name,MeasurementSites$sID)] <- WaterQualityDataUpdate$TP_kg
#Save to a spatial file for external mapping purposes if required
writeOGR(MeasurementSites,dsn=file.path(GISDataDirectory,"ReportLayers"),layer="MeasurementSites",driver="ESRI Shapefile",overwrite_layer=TRUE)
...
#FOr the default Scenario, use the saved LeachRateRasterFile if it exists

if (Scenario == "Default") {
  LeachRateRaster <- stack(LeachRateRasterFile)
  crs(LeachRateRaster) <- sf::st_crs(2193)$wkt
  ...
  crs(LandUseRaster) <- sf::st_crs(2193)$wkt
} else {
  
  #The LeachRateRaster and the LandUseRaster needs to change for the different scenarios.
  #**********************************************************
  # This can also be used if the leach rate raster file is missing or needs to be re-created
  ...
  LeachRateRaster <- LeachRateRasterCreator(LanduseData=LanduseShapeFile,
                                            ...
                                            Resolution = 250,
                                            ScenarioLandUseGISFolder = ScenarioLandUseGISDirectory,
                                            Scenarios = Scenario)
  toc()
  # #And save it for nexttime if this is to replace the default
  ...
  LandUseRaster <- LeachRateRaster[[3]]
  #Change the raster attribute table header to "category", so that it is consistent to the name when loaded direct from a Tiff
  rat <- levels(LandUseRaster); names(rat[[1]])[2] <- "category"; levels(LandUseRaster) <- rat
  LeachRateRaster <- LeachRateRaster[[1:2]]
}
#******************************************************************
...
Remove any duplicate sites or sites manually deemed unnecessary as listed in the "unnecessary" file
...
#Add the River names. Note that the SouthlandREC2Utility names are not very good. So don't use it.
...
SubCatchmentConfluencePointsSpatial$RiverName <- NA
...
#writeOGR(SubCatchmentConfluencePointsSpatial, file.path(GISDataDirectory,"SubCatchmentJunctionSites"),layer="SubcatchmentJunctionSitesV3",driver="ESRI Shapefile",overwrite_layer=TRUE)
```
...
NetworkLabelList <- NetworkLabeller(RECV2Network = AllPointsCompleteSpatialNetwork,
                            ExtraRECReachLabelsFile = ExtraRECReachLabelsFile,
                            EstuarySites = EstuarySites)
...
If a plot of histograms of upstream attributes are required, then this is available in the AttributeDistributionPlot() function, within the SouthlandSCAMPfunctions.R file
#AttributeDistributionPlot(RECReachNetwork = RECReachNetwork,CompleteDomain = CompleteDomain,MajorCatchments = MajorCatchments ,SouthlandREC2Utility = SouthlandREC2Utility)
...
#writeOGR(CompleteSpatialNetwork, dsn=GISDataDirectory,layer="CASM-StreamNetwork-namedV3", driver="ESRI Shapefile",overwrite_layer=TRUE)
...
#Add the mean annual flow to the table. Use the measurement site data, but not for the sites with updated locations. Assume the measurement site flows are correct, unless the nzsegment has changed
MeasurementSiteTable$"Mean flow (m3/s)" <- MeasurementSiteData$MeanFlow[match(MeasurementSiteTable$`Site Name or No`,MeasurementSiteData$sID)]
#For those sites that have updated nzsegment numbers, use the mean flow from the REC2MeanFlow data set
SitesRequiringFlowUpdate <- which(!MeasurementSiteTable$nzsegment %in% MeasurementSiteData$nzsegment)
MeasurementSiteTable$"Mean flow (m3/s)"[SitesRequiringFlowUpdate] <- REC2MeanFlow$QMean[match(MeasurementSiteTable$nzsegment[SitesRequiringFlowUpdate],REC2MeanFlow$nzsegment)]

#Correct the Waiau flows for Manapouri operation. This is no longer needed as the source data has been edited for the two Waiau flow sites.
#This does mean that the flow data from the DIffuse nodes doesn't always match the measurement sites though.
...
#Round the values
MeasurementSiteTable$"Mean flow (m3/s)" <- round(MeasurementSiteTable$"Mean flow (m3/s)",2)
...
#Now it is necessary to prepare the actual diffuse data for the source area of each diffuse source point
...
if (!DiffuseSourceAreasOK) {
  ...
  if(!exists("RECWatersheds")) {
      ...
      RECWatersheds <- st_transform(RECRivers_SF, 2193)
  }
...
#Load the landuse raster data has already been loaded
...
#OldLeachRateRaster <- LeachRateRaster
if (any(Scenario %in% c("OurLandWater2015Potential","OurLandWater2035Potential","PlantainAndOLW"))){
  MitigationScenario = paste0("Potential",gsub("^[a-z,A-Z]*([0-9]+).*$","\\1",Scenario))
  if (any(Scenario %in% c("PlantainAndOLW")) ) MitigationScenario <- "Potential2015"
  #Adjust the LeachRateRasters for the Our Land and Water Mitigation scenarios
  LeachRateRaster <- LeachRateAdjuster(LeachRates = LeachRateRaster,
...
if (any(Scenario %in% c("ESLand"))){
  #Adjust the LeachRateRasters for ESLand
  LeachRateRaster <- LeachRateAdjustmentByShapeFile(LeachRates = LeachRateRaster,
                                       PercentChange = 50,
                                       PolygonShapeFile = ESLandPolygonFile)
  #Need to eventually remove all use of the Raster package objects, but in the interim convert back to a stack object
  LeachRateRaster <- raster::stack(LeachRateRaster)
}
...
Chloropleth mapping is from here https://rstudio.github.io/leaflet/choropleths.html
...
#The Subcatchment points never get labelled properly so do that now.
#Use their river reach names, but need to use the all-points network, not the CASM refined network.
#The CASM refined network doesn't have all the reaches, so not everything can be named. For the small side tributaries this gives them the right name. If it was ignored and the naming was done after the CASM override was done, then the names would transfer to the overriden reach which is often a large river, so you wouldn't really know where it was.
SubCatchmentConfluencePointsSpatial$RiverName <- AllPointsCompleteSpatialNetwork$Label[match(SubCatchmentConfluencePointsSpatial$nzsegment,AllPointsCompleteSpatialNetwork$nzsegment)]

#Create a SCAMP subcatchment polygon spatial file by dissolving DiffuseLoadSourceAreas on SCAMPnzseg
SCAMPSubCatchments <- DiffuseLoadSourceAreas %>% group_by(SCAMPnzseg) %>%
...
SpatialData <- list(MeasurementSites=MeasurementSites,LakeSites=LakeSites,EstuarySites=EstuarySitesSpatial,SubCatchmentSites=SubCatchmentConfluencePointsSpatial,RiverMouthSites=RiverMouthSites, RiverNetwork=RECForMaps,MajorCatchments=MajorCatchments,GroundwaterZones=GroundwaterZones, PointSourceSites=PointSourcesSpatial, TeAoMaramaSites=TeAoMaramaSpatial,SCAMPSubCatchments = as(SCAMPSubCatchments,"Spatial"))

SpatialData <- lapply(SpatialData, function(SingleLayer) {
  SingleLayer$CASMnzsegment <- SingleLayer$nzsegment
  IndicesOfnzsegmentsToUpdate <- which(SingleLayer$CASMnzsegment %in% CASMnzsegmentOverrideData$nzsegment)
SingleLayer$CASMnzsegment[IndicesOfnzsegmentsToUpdate] <- CASMnzsegmentOverrideData$CASMnzsegment[match(SingleLayer$nzsegment[IndicesOfnzsegmentsToUpdate],CASMnzsegmentOverrideData$nzsegment)]
return(SingleLayer)
 }) 
...
crs(MapData$Physiography) <- sf::st_crs(4326)$wkt
...
Note that for Wetland Scenarios this can take 10 minutes!!
```{r}
Out <- createWorkbook()
...
#Diffuse Inputs is no longer needed as an output sheet
...
addStyle(Out,sheet = "Catchment Physical Inputs",cols=4:15,rows=3,style=hs2)
addStyle(Out,sheet = "Catchment Physical Inputs",cols=6,rows=2,style=subheadingstyle1)
addStyle(Out,sheet = "Catchment Physical Inputs",cols=5:15,rows=4:(nrow(SCAMPFormattedDiffuseOutput$AreaTable)+3),gridExpand = TRUE, style=OnsSD)
...
addStyle(Out,sheet = "Catchment WQ Inputs TN",cols=c(4:15),rows=3,style=hs2)
addStyle(Out,sheet = "Catchment WQ Inputs TN",cols=6,rows=2,style=subheadingstyle1)
addStyle(Out,sheet = "Catchment WQ Inputs TN",cols=5:15,rows=4:(nrow(SCAMPFormattedDiffuseOutput$NTable)+3),gridExpand = TRUE, style=OnsSD)
...
addStyle(Out,sheet = "Catchment WQ Inputs TP",cols=c(4:15),rows=3,style=hs2)
addStyle(Out,sheet = "Catchment WQ Inputs TP",cols=6,rows=2,style=subheadingstyle1)
addStyle(Out,sheet = "Catchment WQ Inputs TP",cols=5:15,rows=4:(nrow(SCAMPFormattedDiffuseOutput$NTable)+3),gridExpand = TRUE, style=OnsSD)

if (any(Scenario %in% c("Wetland1","Wetland3"))){

  addWorksheet(Out, "Wetland Inputs")
 WetlandData <- NewWetlandProperties(RECReachNetwork = RECReachNetwork,
                                      RECWatersheds = paste0(REC_Catchments,".shp"),
                                      NewWetlandSpatial = file.path(ScenarioLandUseGISDirectory,paste0(Scenario[1],'WithExtraAttributes.shp')),
                                      LandUseRaster = LandUseRaster,
                                      LandUseLUT = LanduseLUT,
                                      SubCatchmentSpatial = DiffuseLoadSourceAreas)
  #Fix catchment names by looking up the catchment name based on the SCAMP_nzsegment number
  CatchmentNames <- SCAMPFormattedDiffuseOutput$AreaTable$'Catchment Name'
  Catchment_SCAMPnzsegment <- sub('(^[0-9]+).*','\\1',CatchmentNames)
  WetlandData$'Catchment Name' <- CatchmentNames[match(WetlandData$'Catchment Name',Catchment_SCAMPnzsegment)]

  writeData(Out,sheet = "Wetland Inputs",x = WetlandData, startRow = 3, headerStyle = hs1)
writeData(Out,sheet = "Wetland Inputs",x = 'Landuse Distribution (%)', xy = c(4,2))
setColWidths(Out,sheet = "Wetland Inputs",cols =c(1,2,17,18),widths = 'auto')
addStyle(Out,sheet = "Wetland Inputs",cols=3:18,rows=3,style=hs2)
addStyle(Out,sheet = "Wetland Inputs",cols=4,rows=2,style=subheadingstyle1)
addStyle(Out,sheet = "Wetland Inputs",cols=3:18,rows=4:(nrow(WetlandData)+3),gridExpand = TRUE, style=OnsSD)
}

saveWorkbook(Out, OutputFileName, overwrite = T)
```


If you want to check how different the output spreadsheet is to previous spreadsheets then use the 
```{r, eval = FALSE}
bob <- LoadComparerV2(c(file.path(DataDirectory,"SCAMP-InputsV4.xlsx"),OutputFileName))
sum(bob$AbsDiff)
#Need to write a new comparer for SCAMP input files without the Diffuse Inputs sheet.
```