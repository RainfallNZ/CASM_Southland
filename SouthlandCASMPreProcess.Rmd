...
Rasterise the Southland REC vector layer to 8 m x 8m resolution
```{r}
REC3Watersheds <- readOGR(dsn = GISDataDirectory,layer = "ESWaterShedsWithPlanClasses")
  #Convert to raster, note the creation of a base raster, which all subsequent raster's align to
RasterBase <- raster(resolution = 8, ext = extent(REC3Watersheds), crs = proj4string(REC3Watersheds) )
  REC3WatershedsRaster <- rasterize(REC3Watersheds,RasterBase,field="WaterPlanC",fun=max)
   writeRaster(TotalRaster,file.path(GISDataDirectory,"REC3WaterPlan_8m.tiff"),overwrite=TRUE)
   
   REC2Watersheds <- readOGR(dsn = GISDataDirectory,layer = "ESWaterShedsWithPlanClasses")
```
