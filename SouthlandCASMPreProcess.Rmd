...
NZsegmentToWaterPlanClassLookUpTableFile <-  "NZsegmentToWaterPlan.csv"
...
NZsegmentToWaterPlanClassLookUpTable <- read.table(file.path(DataDirectory,NZsegmentToWaterPlanClassLookUpTableFile), sep=",", stringsAsFactors = FALSE,header = TRUE)

#If the lookup table is not available then it can be recreated using the code immediately below
# {
# #Read in the REC riverlines that have bee converted to points
...
#RECReachNetwork$WaterPlan <- NZsegmentToWaterPlanClassLookUpTable$Name[match(RECReachNetwork$nzsegment, NZsegmentToWaterPlanClassLookUpTable$nzsegment)]
...
***Need to do the full processing from REC3 to REC2 here
rasterize REC3

Can't get this to work efficiently yet !! buffer on majority of nearest neighbor seems to take too long.
sample REC2 points from raster
agregate points on nzsegment and majority of the REC3 attribute

```{r}
#Load the high resolution Water Plan Class raster
# WaterPlanRasterHiRes <- raster(file.path(GISDataDirectory,"WaterPlan_8m.tif"))
# 
# Mode <- function(x) {
#   ux <- unique(na.omit(x))
...
#   tic()
# 
# WaterPlanRasterHiResNAFilled <- focal(WaterPlanRasterHiRes, w = matrix(1,3,3), fun= fill.na, pad=TRUE, na.rm=FALSE)
# writeRaster(WaterPlanRasterHiResNAFilled,file.path(GISDataDirectory,"WaterPlanRasterNAFilled.tiff"))
# toc()
# }
...
RECForMaps <- RECReachNetwork[(sqrt(RECReachNetwork@data$headw_dist)/1000 > 1) | (RECReachNetwork@data$nzsegment %in% CompleteNetwork),]
RECForMaps$LineWidthPixels <- pmax(1,as.integer(sqrt(RECForMaps@data$headw_dist)/1000)) #SCale the line width, with a minimum of 1 pixel

RECForMaps <- RECForMaps[,c("LineWidthPixels","WaterPlanC")]
RECForMaps$WaterPlan <- as.factor(RECForMaps$WaterPlanC)
