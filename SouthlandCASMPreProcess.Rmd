...
#Add the mean annual flow to the table. Use the measureent site data, but not for the sites with updated locations. Assume the measurement site flows are correct, unless the nzsegment has changed
MeasurementSiteTable$"Mean flow (m3/s)" <- round(MeasurementSiteData$MeanFlow[match(MeasurementSiteTable$`Site Name or No`,MeasurementSiteData$sID)])
...
#Round the values
MeasurementSiteTable$"Mean flow (m3/s)" <- round(MeasurementSiteTable$"Mean flow (m3/s)",1)
...
saveWorkbook(Out, file.path(DataDirectory,"CASM-InputsV3.xlsx"), overwrite = T)
```
