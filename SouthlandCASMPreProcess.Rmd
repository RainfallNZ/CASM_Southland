...
#Load a maually prepared update file which has manual estimates for corrected attributes for some segments
...
#When the Landuse raster is created from the PredictorRasterFile, the Raster Attribute Table has an "ID" and a "category".
#When it is saved directly during its creation (usually when updating scenarios), the Raster Attribute Table has an "ID" and a "LanduseClass". This is because the *.tiff format doesn't retain raster attribute table names.
#for compatability between the two possible sources, make sure the land use name column is called "category".
names(LanduseLUT)[2] <- "category"
